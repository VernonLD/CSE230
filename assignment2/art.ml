...
(* build: (int*int->int) * int -> Expr 
   ...
   the expression.
*)
(* Rand funcion takes a tuple (a,b), and generate a random number between a and b *)
(* let rand (a,b) = Random.int (b - a) + a;; *)
...
let rec build2 (rand,depth) = 
  ...
  | _ ->  match rand (1,8) with
           | 1 -> buildSine(build2 (rand, depth - 1))
           | 2 -> buildCosine(build2 (rand, depth - 1))
           | 3 -> buildAverage(build2 (rand, depth - 1),build2 (rand, depth - 1))
           | 4 -> buildTimes(build2 (rand, depth - 1),build2 (rand, depth - 1))
           | 5 -> buildSub(build2 (rand, depth - 1),build2 (rand, depth - 1))
           | 6 -> buildNeg(build2 (rand, depth - 1),build2 (rand, depth - 1),build2 (rand, depth - 1))
